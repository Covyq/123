...
def has_time_manager_role(ctx: ApplicationContext) -> bool:
    """Проверяет, есть ли у пользователя роль Тайм.менеджер"""
    return any(role.name == "Тайм.менеджер" for role in ctx.author.roles)
...
async def setskladchannel_command(ctx: ApplicationContext,
    ...
    if not has_time_manager_role(ctx):
        await ctx.respond("❌ Только для Тайм.менеджер.", ephemeral=True)
        return
...
async def channels_command(ctx: ApplicationContext):
    with db:
        ...
        sklad_channels = Table_SkladChannel.select().where(Table_SkladChannel.guild_id == ctx.guild.id)
    
    simple_m = ctx.guild.get_channel(simple_id).mention if simple_id and ctx.guild.get_channel(simple_id) else "не задан"
    
    sklad_info = ""
    for sc in sklad_channels:
        with db:
            hex_obj = Table_Hex.get_by_id(sc.hex_id)
        ch = ctx.guild.get_channel(sc.channel_id)
        mention = ch.mention if ch else f"<#{sc.channel_id}>"
        sklad_info += f"🏭 {hex_obj.hex_value}: {mention}\n"
    
    if not sklad_info:
        sklad_info = "не задано"
    
    await ctx.respond(
        f"📋 **Каналы бота:**\n{sklad_info}🕐 Таймеры: {simple_m}",
        ephemeral=True
    )
