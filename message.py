...
async def loop():
    ...
    for t in expired:
        try:
            ...
            if t.kind == "mpf":
                await msg.edit(content=f"✅ ЗАКРЫТО\n{t.text}\n⌛ завершено {mention}")
            else:
                await msg.edit(content=f"✅ {t.text} завершён {mention}\n⏰ <t:{now}:R>")
...
# ─── ТВОЙ ОРИГИНАЛЬНЫЙ /ТАЙМЕР (ВОЗВРАЩЁН) ─────────────
...
# ─── ТВОЙ ОРИГИНАЛЬНЫЙ /СКЛАД (ВОЗВРАЩЁН) ──────────────
