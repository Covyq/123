...
def format_sklad_text(username: str, hex_val: str, region: str, warehouse: str, password: str) -> str:
    return (
        f"**Никнейм:** {username}\n"
        f"**Гекс:** {hex_val}\n"
        f"**Регион:** {region}\n"
...
@DSBot.slash_command(name="setskladchannel", guild_ids=[GUILD_ID],
    description="Установить канал для таймеров склада (только time.manager)")
async def setskladchannel_command(ctx: ApplicationContext,
    channel: Option(SlashCommandOptionType.channel,
    description="Канал", channel_types=[ChannelType.text])):
    # Проверка роли time.manager
    has_role = any(role.name == "time.manager" for role in ctx.author.roles)
    if not has_role:
        await ctx.respond("❌ Только для пользователей с ролью time.manager.", ephemeral=True)
        return
    with db:
...
@DSBot.slash_command(name="setsimplechannel", guild_ids=[GUILD_ID],
    description="Установить канал для обычных таймеров (только администраторы)")
...
async def timer_command(ctx: ApplicationContext,
    ...
    # Фиксированное время: 2 дня + 1 час
    ...
    with db:
        Table_SkladTimer.create(guild_id=ctx.guild.id, channel_id=ctx.channel.id,
            ...
            time_shift=(2 * 86400 + 3600), time_end=time_end)
