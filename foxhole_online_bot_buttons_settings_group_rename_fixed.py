...
async def build_online_embed(guild):
    ...
    for section in ["command", "main", "experienced"]:
        ...
        if section_fields:
            ...
            active_groups_count = len(section_fields)

            for title, members in section_fields:
                embed.add_field(
                    name=make_group_field_name(section, title, len(members), active_groups_count),
...
def build_hierarchy_embed(guild_id):
    ...
    embed = discord.Embed(
        ...
        description=(
            ...
            "Группы и роли хранятся в базе данных и настраиваются через Discord-команды.\n\n"
            "**Разделы для команд:**\n"
            "`command` — Командование\n"
