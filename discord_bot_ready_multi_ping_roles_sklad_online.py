...
# Роли, которые будут отображаться в /онлайн.
# Формат:
# ID роли: "Название, которое будет видно в embed"
#
# ВАЖНО:
# Если человек имеет несколько ролей из списка, он будет показан в первой подходящей роли
# по порядку списка ниже, чтобы не было дублей.
ONLINE_TRACKED_ROLES = {
    1501797623550578889: "🛡️ Командование",
    422500854910681089: "⭐ Офицеры",
    1224787828815171595: "⚔️ Старший состав",
    1501799982389264494: "🎖️ Лейтенанты",
    1501800302024720464: "🪖 Пехота",
    1501800617176203444: "💣 Артиллерия",
    1501801258594467870: "🚚 Логистика",
    1501801774414299198: "🏭 Завод",
    1501801928915419269: "🕵️ ДРГ",
    1501798393909870652: "🛡️ Танкисты",
    1501799300387307540: "✈️ Лётчики",
    1501799681024462980: "🧭 Рекруты",
    1501800738312159313: "🎥 Медиа",
    1501801036539756635: "📌 Другое",
    1210544000600375297: "👥 Участники",
}
...
def get_tracked_foxhole_players_by_role(guild):
    """
    Возвращает словарь:
    role_id -> [member, member, ...]

    Если у человека несколько отслеживаемых ролей, он попадёт только в первую
    подходящую роль по порядку ONLINE_TRACKED_ROLES.
    """
    players_by_role = {role_id: [] for role_id in ONLINE_TRACKED_ROLES.keys()}
    ...
    for member in guild.members:
        ...
        for role_id in ONLINE_TRACKED_ROLES.keys():
            if role_id in member_role_ids:
                players_by_role[role_id].append(member)
                break

    for role_id in players_by_role:
        players_by_role[role_id].sort(key=lambda m: m.display_name.lower())

    return players_by_role


def build_online_embed(guild, players_by_role, updated_ts):
    total = sum(len(players) for players in players_by_role.values())
    ...
    for role_id, role_title in ONLINE_TRACKED_ROLES.items():
        role = guild.get_role(role_id) if guild else None
        role_display_name = role_title

        if role:
            role_display_name = f"{role_title} / {role.name}"

        players = players_by_role.get(role_id, [])
        ...
        embed.add_field(
            name=f"{role_display_name} — {len(players)}",
            value=value,
            inline=False,
        )
...
async def update_online_embed_for_guild(guild, now_ts):
    ...
    players_by_role = get_tracked_foxhole_players_by_role(guild)
    embed = build_online_embed(guild, players_by_role, now_ts)
