...
# СТАРТОВЫЕ ГРУППЫ
# =========================
# Эти группы нужны только для первого запуска.
# Бот сам перенесёт их в базу данных.
# После этого группы и роли можно менять командами Discord, не открывая код.

DEFAULT_ONLINE_GROUPS = [
    {
        "section": "command",
        "title": "👑 Капитан",
    ...
    },
    {
        "section": "activity",
        "title": "✈️ Пилот",
        "role_ids": [1463921076433064227],
        "position": 1,
    },
    {
        "section": "activity",
        "title": "🦊 Партизан",
        "role_ids": [1501768722304598167],
        "position": 2,
    },
    {
        "section": "activity",
        "title": "🔫 Пехота",
        "role_ids": [1241513004928077885],
        "position": 3,
    },
    {
        "section": "activity",
        "title": "💥 Артиллерия",
        "role_ids": [1241663050776580189],
        "position": 4,
    },
    {
        "section": "activity",
        "title": "🛡️ Танк",
        "role_ids": [1241512966235623505],
        "position": 5,
    },
    {
        "section": "activity",
        "title": "🚢 Флот",
        "role_ids": [1315281120400638034],
        "position": 6,
    },
    {
        "section": "activity",
        "title": "🌊 Подводник",
        "role_ids": [1463921275775877184],
        "position": 7,
    },
    {
        "section": "activity",
        "title": "📦 Логист",
        "role_ids": [1194748526131945502],
        "position": 8,
    },
    {
        "section": "activity",
        "title": "🏗️ Заводчанин-строитель",
        "role_ids": [1241513032304295947],
        "position": 9,
    },
    {
        "section": "activity",
        "title": "🚨 QRF",
        "role_ids": [1390688779056058429],
        "position": 10,
    },
    {
        "section": "activity",
        "title": "🛩️ Авиация",
        "role_ids": [1463925850654248991],
        "position": 11,
    },
]
...
# Если она не задана, бот проверяет все роли из онлайн-групп.
...
def seed_default_role_groups(guild_id):
    exists = OnlineRoleGroup.select().where(OnlineRoleGroup.guild_id == guild_id).exists()

    if exists:
        return

    with db.atomic():
        for group in DEFAULT_ONLINE_GROUPS:
            OnlineRoleGroup.create(
                guild_id=guild_id,
                section=group["section"],
                title=group["title"],
                role_ids_json=json.dumps(group["role_ids"]),
                position=group["position"],
            )

    logger.info(f"Стартовые онлайн-группы созданы для guild_id={guild_id}")
...
def get_groups_by_section(guild_id):
    result = {section: [] for section in VALID_GROUP_SECTIONS}
...
async def build_online_embed(guild):
    ...
    for section in ["command", "main", "experienced"]:
        ...
        if section_fields:
            add_section_header(embed, VALID_GROUP_SECTIONS[section])
    ...
    if activity_fields:
        add_section_header(embed, VALID_GROUP_SECTIONS["activity"])
    ...
    if not online_members:
        embed.add_field(
            name="━━━━━━━━━━━━━━",
            value="Сейчас никто из отслеживаемых ролей не найден в Foxhole.",
            inline=False,
        )
...
def build_hierarchy_embed(guild_id):
    ...
    embed = discord.Embed(
        ...
        description=(
            ...
            "Группы и роли теперь хранятся в базе данных, а не только в коде.\n\n"
            "**Разделы для команд:** `command`, `main`, `experienced`, `activity`"
        ),
    )

    for section in ["command", "main", "experienced", "activity"]:
        lines = []
        ...
        for index, chunk in enumerate(chunk_text("\n\n".join(lines), max_len=1000), start=1):
            name = VALID_GROUP_SECTIONS[section]
            if len(lines) > 1:
...
async def on_ready():
    ...
    try:
        ...
        logger.error("Ошибка создания стартовых онлайн-групп:")
        logger.error(traceback.format_exc())
