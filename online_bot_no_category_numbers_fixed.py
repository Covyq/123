...
async def онлайн(
    ctx,
    канал: discord.Option(discord.TextChannel, "Канал для сообщения онлайна", required=False) = None,
    айди_ветки: discord.Option(str, "ID ветки/треда, если нужно отправлять онлайн в ветку", required=False) = None,
):
    if not can_manage_online(ctx.author):
    ...
    if канал:
        ...
        try:
            target_id = int(айди_ветки.strip())
        except ValueError:
