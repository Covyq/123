...
# РАЗДЕЛЫ ГРУПП
# =========================
# Группы и роли создаются и настраиваются через команды Discord:
...
def normalize_title_for_compare(text):
    # Нужна, чтобы не дублировать название раздела и группы.
    # Например: раздел "🎖️ Основной состав" и группа "Основной состав".
    cleaned = re.sub(r"[^0-9A-Za-zА-Яа-яЁё ]+", "", text or "")
    cleaned = re.sub(r"\s+", " ", cleaned).strip().lower()
    return cleaned


def make_group_field_name(section, group_title, members_count, active_groups_count):
...
async def build_online_embed(guild):
    ...
    # Основные группы показываются без отдельных разделов.
    # Человек отображается только один раз — в самой высокой подходящей группе.
    for section in ["command", "main", "experienced"]:
        for group in groups_by_section.get(section, []):
    ...
    # Группы специализаций тоже показываются без отдельного заголовка.
    # В activity игрок может отображаться повторно, если имеет несколько ролей.
...
def build_hierarchy_embed(guild_id):
    ...
    embed = discord.Embed(
        ...
        description=(
            ...
            "Группы и роли хранятся в базе данных и настраиваются через Discord.\n\n"
            "Отдельные разделы в отображении онлайна скрыты — показываются только сами группы."
        ),
    )
    ...
    for section in SECTION_ORDER:
        for group in groups_by_section.get(section, []):
            ...
            lines.append(
                f"`ID {group.id}` | `Место {group.position}` | {group.title}\n"
                f"{roles_text}"
            )
...
def build_settings_help_embed(guild_id):
    ...
    embed.description = (
        ...
        "Где `section` — раздел, `название` — название группы, `роль` — Discord-роль.\n\n"
        ...
        "Удаляет выбранную роль из указанной группы.\n\n"
        "**Поменять порядок группы:**\n"
        "`/online_group_move group_id:12 position:1`\n"
        "Где `position` — место группы в разделе. Чем меньше число, тем выше группа.\n\n"
        ...
        "После этого список без Steam будет проверяться только по этой роли.\n\n"
...
class AddGroupModal(discord.ui.Modal):
    def __init__(self):
        ...
        self.add_item(discord.ui.InputText(
            label="Тип группы",
            placeholder="command / main / experienced / activity",
            required=True,
            max_length=32,
        ))
        self.add_item(discord.ui.InputText(
    ...
    async def callback(self, interaction):
        ...
        section = normalize_section(self.children[0].value)
        title = self.children[1].value.strip()
        role_id = parse_discord_id(self.children[2].value)

        if not section:
            return await interaction.response.send_message(
                "❌ Неверный тип группы. Используй: `command`, `main`, `experienced`, `activity`.",
                ephemeral=True,
            )
        ...
        group = OnlineRoleGroup.create(
            ...
            position=get_next_position(interaction.guild.id, section),
        )
...
class MoveGroupModal(discord.ui.Modal):
    ...
    async def callback(self, interaction):
        ...
        group_id = parse_positive_int(self.children[0].value)
        position = parse_discord_id(self.children[1].value)
...
class MoveGroupSectionModal(discord.ui.Modal):
    def __init__(self):
        super().__init__(title="Перенести группу в раздел")
        self.add_item(discord.ui.InputText(
            label="ID группы",
            placeholder="Например: 12",
            required=True,
    ...
    async def callback(self, interaction):
        ...
        group_id = parse_positive_int(self.children[0].value)
        section = normalize_section(self.children[1].value)

        if not group_id or not section:
            return await interaction.response.send_message(
                "❌ Укажи корректный ID группы и раздел: `command`, `main`, `experienced`, `activity`.",
                ephemeral=True,
            )
        ...
        group.position = get_next_position(interaction.guild.id, section)
        group.save()
        await refresh_online_after_setting(interaction)
        await interaction.response.send_message(
            f"✅ Тип группы изменён.\nГруппа: {group.title}\nБыло: `{old_section}`\nСтало: `{section}`",
            ephemeral=True,
        )
...
class OnlineSettingsButtonView(discord.ui.View):
    ...
    @discord.ui.button(label="📂 Раздел", style=discord.ButtonStyle.gray)
    async def section_group_button(self, button, interaction):
        if not can_manage_online(interaction.user):
            return await deny_interaction(interaction)
        await interaction.response.send_modal(MoveGroupSectionModal())
...
async def online_group_add(
    ...
    await ctx.respond(
        ...
        f"Тип группы: `{normalized_section}`\n"
