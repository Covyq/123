# discord_bot_ready_multi_ping_roles_sklad_online.py
...
# 6) Для отслеживания игры Foxhole через Discord нужны включённые Intents в Discord Developer Portal:
...
#
# ВАЖНО:
# 1) Впиши в role_ids реальные ID ролей Discord-сервера.
# 2) Если человек подходит под несколько основных ролей, он попадёт только
#    в первую подходящую группу по порядку списка ниже.
# 3) В разделе "Роды деятельности" человек может отображаться сразу
#    в нескольких группах, если у него несколько соответствующих ролей.

ONLINE_RANK_ROLE_GROUPS = [
    {
        "title": "Капитан",
        "role_ids": [
            1502970847877402776,
    ...
    },
    {
        ...
        "role_ids": [
            # ВПИШИ ID РОЛИ СЕРЖАНТСКОГО СОСТАВА
        ],
    },
    {
        "title": "Бойцы",
    ...
    },
    {
        "title": "Свежая кровь",
        "role_ids": [
            # ВПИШИ ID РОЛИ СВЕЖАЯ КРОВЬ
        ],
    },
]

ONLINE_ACTIVITY_ROLE_GROUPS = [
    {
        "title": "С ролью пехота",
        "role_ids": [
            1502969601754136576,
        ],
    },
    {
        "title": "С ролью артиллерия",
        "role_ids": [
            # ВПИШИ ID РОЛИ АРТИЛЛЕРИЯ
        ],
    },
    {
        "title": "С ролью Танк",
    ...
    {
        "title": "С ролью Подводник",
        "role_ids": [
            # ВПИШИ ID РОЛИ ПОДВОДНИК
        ],
    },
    {
        "title": "C роль партизан",
        "role_ids": [
            1502971638684061806,
        ],
    },
    ...
    {
        "title": "С ролью логист",
        "role_ids": [
            # ВПИШИ ID РОЛИ ЛОГИСТ
        ],
    },
    {
        "title": "C ролью заводчанин",
        "role_ids": [
            # ВПИШИ ID РОЛИ ЗАВОДЧАНИН
        ],
    },
    {
        "title": "С ролью QRF",
...
# Для совместимости старого названия внутри кода.
ONLINE_TRACKED_ROLE_GROUPS = ONLINE_RANK_ROLE_GROUPS
...
def build_empty_players_by_group(groups):
    return {
...
def format_online_members(players):
    if not players:
        return "—"

    value = "\n".join(f"• {member.mention}" for member in players[:25])
...
async def get_tracked_foxhole_players_by_group(guild):
    ...
    ranks -> основные роли клана;
    activities -> роды деятельности.

    Игрок считается онлайн в Foxhole только если Discord activity показывает Foxhole.

    В основных ролях игрок показывается только в первой подходящей группе.
    В родах деятельности игрок может отображаться сразу в нескольких группах.
    """
    players_by_rank = build_empty_players_by_group(ONLINE_RANK_ROLE_GROUPS)
    players_by_activity = build_empty_players_by_group(ONLINE_ACTIVITY_ROLE_GROUPS)

    if not guild:
        return {"ranks": players_by_rank, "activities": players_by_activity}
    ...
    for member in guild.members:
        ...
        for index, group in enumerate(ONLINE_RANK_ROLE_GROUPS):
            group_role_ids = set(group.get("role_ids", []))

            if member_role_ids & group_role_ids:
                players_by_rank[get_online_group_key(index)].append(member)
                break

        for index, group in enumerate(ONLINE_ACTIVITY_ROLE_GROUPS):
            group_role_ids = set(group.get("role_ids", []))

            if member_role_ids & group_role_ids:
                players_by_activity[get_online_group_key(index)].append(member)

    for group_key in players_by_rank:
        players_by_rank[group_key].sort(key=lambda m: m.display_name.lower())

    for group_key in players_by_activity:
        players_by_activity[group_key].sort(key=lambda m: m.display_name.lower())

    return {"ranks": players_by_rank, "activities": players_by_activity}


def build_online_embed(guild, players_by_group, updated_ts):
    rank_groups = players_by_group.get("ranks", {})
    activity_groups = players_by_group.get("activities", {})

    unique_online_members = set()
    for players in rank_groups.values():
        unique_online_members.update(member.id for member in players)

    total = len(unique_online_members)
    ...
    embed = discord.Embed(
        ...
        description=(
            ...
            "Источник: Discord activity.\n"
            "Никнеймы отображаются через @упоминание."
        ),
    )
    ...
    for index, group in enumerate(ONLINE_RANK_ROLE_GROUPS):
        ...
        players = rank_groups.get(group_key, [])

        embed.add_field(
            name=f"{group_title} — {len(players)}",
            value=format_online_members(players),
            inline=False,
        )
    ...
    for index, group in enumerate(ONLINE_ACTIVITY_ROLE_GROUPS):
        group_key = get_online_group_key(index)
        group_title = group.get("title", f"Роль {index + 1}")
        players = activity_groups.get(group_key, [])
...
async def set_online_channel(
    ...
    await ctx.respond(
        ...
        "Источник онлайна: Discord activity.",
        ephemeral=True,
    )
