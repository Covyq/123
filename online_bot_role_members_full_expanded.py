...
def build_active_fighters_embed(guild, days=7):
    seed_default_role_groups(guild.id)
    tracked_role_ids = get_all_role_ids_from_groups(guild.id)
    ...
    active_rows = list(
        ...
        .order_by(OnlineMemberPresence.last_seen_at.desc())
    )
    ...
    if active_rows:
        ...
        for row in active_rows[:40]:
            ...
            lines.append(f"{name} — последний раз `{last_seen}`")

        for index, chunk in enumerate(chunk_text("\n".join(lines), max_len=1000), start=1):
            embed.add_field(name=f"Активные | часть {index}", value=chunk, inline=False)
    ...
    embed.set_footer(text="Активность считается по факту появления в Foxhole через Discord Activity или Steam API")
    embed.timestamp = utc_now()
    return embed
...
def build_settings_help_embed(guild_id):
    ...
    embed.description = (
        ...
        "`/online_month_graph` или кнопка `📈 График месяца`\n\n"
        "**Активные бойцы:**\n"
        "кнопка `🪖 Активные бойцы` или `/active_fighters days:7`\n\n"
...
class SettingsPanelView(discord.ui.View):
    ...
    async def clear_steam_role_button(self, button, interaction):
        ...
        await log_admin_action(interaction.guild, interaction.user, "steam_missing_role_clear", "Через кнопку настройки")
        await interaction.response.send_message(
...
class OnlineControlView(discord.ui.View):
    ...
    async def active_fighters_button(self, button, interaction):
        ...
        embed = build_active_fighters_embed(interaction.guild, days=7)
        await interaction.response.send_message(embed=embed, ephemeral=True)

    @discord.ui.button(
        label="📈 График месяца",
        style=discord.ButtonStyle.blurple,
        custom_id="online_panel:month_graph",
    )
    async def month_graph_button(self, button, interaction):
        if not can_manage_online(interaction.user):
            return await deny_interaction(interaction)

        await interaction.response.defer(ephemeral=True)
        path, error = generate_monthly_online_graph(interaction.guild.id)
        if error:
...
async def steam_missing_role_clear(ctx):
    ...
    await log_admin_action(ctx.guild, ctx.author, "steam_missing_role_clear", "Роль проверки Steam очищена")
...
async def active_fighters(ctx, days: int = 7):
    if not can_manage_online(ctx.author):
