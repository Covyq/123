...
# =========================
# ЕДИНАЯ ИЕРАРХИЯ ГРУПП
# =========================
# Разделов больше нет. Все онлайн-группы идут единым списком по полю position.
# Новые группы создаются через кнопку ⚙️ Настройка или команду /online_group_add.
DEFAULT_GROUP_TYPE = "default"
...
def get_role_mention_or_id(guild, role_id):
...
# ГРУППЫ РОЛЕЙ В БАЗЕ
# =========================
def normalize_group_type(value=None):
    # Разделы отключены. Поле section в базе оставлено только для совместимости
    # со старыми записями OnlineDatabase.db.
...
def seed_default_role_groups(guild_id):
    ...
    # Стартовые группы больше не создаются из кода.
...
def get_online_role_groups(guild_id, section=None):
    # section больше не используется: все группы идут единым списком.
    return list(
        OnlineRoleGroup
        .select()
...
def get_groups_by_section(guild_id):
    # Функция оставлена для совместимости со старым кодом.
    # Теперь все группы лежат в одном списке DEFAULT_GROUP_TYPE.
    return {DEFAULT_GROUP_TYPE: get_online_role_groups(guild_id)}


def normalize_group_positions(guild_id):
    # Автоматически приводит порядок к единой иерархии: 1, 2, 3, 4...
    # Это исправляет старые базы, где раньше позиции считались отдельно по разделам.
    ...
    for index, group in enumerate(groups, start=1):
        ...
        if group.section != DEFAULT_GROUP_TYPE:
            group.section = DEFAULT_GROUP_TYPE
            changed = True
...
def get_online_groups(guild_id):
    return normalize_group_positions(guild_id)


def get_next_position(guild_id, section=None):
    row = (
        OnlineRoleGroup
...
def get_member_role_priority(member, online_groups):
    priority = 0
    member_role_ids = {role.id for role in member.roles}

    for group in online_groups or []:
...
def sort_members_by_hierarchy(members, online_groups=None):
    if online_groups is None:
        return sorted(members, key=lambda member: member.display_name.lower())

    return sorted(
        members,
        key=lambda member: (
            get_member_role_priority(member, online_groups),
            member.display_name.lower(),
        ),
    )


def get_mentions(members, online_groups=None):
    if not members:
        return "—"

    members = sort_members_by_hierarchy(members, online_groups)
...
def get_members_by_roles(online_members, role_ids):
    ...
    if (
        ...
        and normalize_title_for_compare(group_title) == normalize_title_for_compare(section_title)
    ):
        return f"Участники — {members_count}"

    return f"{group_title} — {members_count}"


async def build_online_embed(guild):
    seed_default_role_groups(guild.id)
    online_groups = get_online_groups(guild.id)
    ...
    # Единая иерархия групп: группа 1, группа 2, группа 3 и так далее.
    # Игрок показывается только в первой подходящей группе по порядку.
    for group in online_groups:
        ...
        embed.add_field(
            name=f"{group.position}. {group.title} — {len(members)}",
            value=get_mentions(members, online_groups),
            inline=False,
        )

    if online_members and not used_member_ids:
        embed.add_field(
            name="Группы без совпадений",
            value="Онлайн найден, но ни одна настроенная группа не содержит этих пользователей.",
            inline=False,
        )
    ...
    if not online_members:
        embed.add_field(
            ...
            value=(
                ...
                "Если группы ещё не настроены, используй кнопку ⚙️ Настройка или `/online_group_add`."
            ),
            inline=False,
        )
...
async def get_members_without_steam(guild):
    ...
    return sort_members_by_hierarchy(missing, online_groups)
...
def build_hierarchy_embed(guild_id):
    ...
    online_groups = get_online_groups(guild_id)

    embed = discord.Embed(
        title="⚙️ Иерархия онлайн-групп",
        color=discord.Color.dark_gold(),
        description=(
            "Разделов больше нет. Ниже показана единая иерархия групп: "
            "группа 1, группа 2, группа 3 и так далее. "
            "Чем меньше место, тем выше группа в онлайне."
        ),
    )
    ...
    if not lines:
        ...
        for index, chunk in enumerate(chunk_text("\n\n".join(lines), max_len=1000), start=1):
            embed.add_field(
                name=f"Онлайн-группы | часть {index}",
                value=chunk,
                inline=False,
            )
...
def build_settings_help_embed(guild_id):
    ...
    embed.description = (
        "Разделов больше нет. Все группы идут единым списком по иерархии.\n\n"
        "**Добавить новую группу:**\n"
        "`/online_group_add название:🏅 Ветераны роль:@Роль`\n"
        "Где `название` — название группы, `роль` — Discord-роль. Роль можно не указывать.\n\n"
        "**Добавить роль в существующую группу:**\n"
        "`/online_group_role_add group_id:12 роль:@Роль`\n"
        "Где `group_id` — ID группы, а `роль` — роль, которую нужно добавить.\n\n"
        ...
        "Где `position` — место группы в общей иерархии. Чем меньше число, тем выше группа.\n\n"
...
# =========================
# КНОПОЧНАЯ НАСТРОЙКА ГРУПП
# =========================
async def refresh_online_after_setting(interaction):
    try:
        await update_online_for_guild(interaction.guild, save_stats=False)
    except Exception:
        logger.error("Ошибка обновления онлайна после настройки:")
        logger.error(traceback.format_exc())


def get_group_for_interaction(interaction, group_id):
    return OnlineRoleGroup.get_or_none(
...
class AddGroupModal(discord.ui.Modal):
    def __init__(self):
        super().__init__(title="Добавить онлайн-группу")
        self.add_item(discord.ui.InputText(
            label="Название группы",
            placeholder="Например: 🏅 Ветераны",
            required=True,
            max_length=100,
        ))
        self.add_item(discord.ui.InputText(
            label="ID роли или упоминание роли",
            placeholder="Например: @Ветеран или 123456789012345678",
            required=False,
            max_length=100,
        ))
    ...
    async def callback(self, interaction):
        ...
        role_id = parse_discord_id(self.children[1].value)

        role_ids = []
        if role_id:
            role = interaction.guild.get_role(role_id)
            if not role:
                return await interaction.response.send_message("❌ Роль с таким ID не найдена на сервере.", ephemeral=True)
            role_ids.append(role_id)

        group = OnlineRoleGroup.create(
            guild_id=interaction.guild.id,
            section=DEFAULT_GROUP_TYPE,
            title=title,
            role_ids_json=json.dumps(role_ids),
            position=get_next_position(interaction.guild.id),
        )

        await refresh_online_after_setting(interaction)
        await interaction.response.send_message(
            f"✅ Группа создана.\nID группы: `{group.id}`\nМесто в иерархии: `{group.position}`\nНазвание: {group.title}",
            ephemeral=True,
        )


class AddRoleToGroupModal(discord.ui.Modal):
    def __init__(self):
        super().__init__(title="Добавить роль в группу")
        self.add_item(discord.ui.InputText(
            label="ID группы",
        ...
        self.add_item(discord.ui.InputText(
            label="ID роли или упоминание роли",
            placeholder="Например: @Пехота или 123456789012345678",
            required=True,
            max_length=100,
        ))
    ...
    async def callback(self, interaction):
        ...
        group_id = parse_positive_int(self.children[0].value)
        role_id = parse_discord_id(self.children[1].value)

        if not group_id or not role_id:
            return await interaction.response.send_message("❌ Укажи корректный ID группы и ID/упоминание роли.", ephemeral=True)

        group = get_group_for_interaction(interaction, group_id)
        if not group:
        ...
        if role.id in role_ids:
            return await interaction.response.send_message(f"ℹ️ Роль {role.mention} уже есть в группе {group.title}.", ephemeral=True)

        role_ids.append(role.id)
        set_role_ids_for_group(group, role_ids)
        await refresh_online_after_setting(interaction)
        await interaction.response.send_message(f"✅ Роль {role.mention} добавлена в группу {group.title}.", ephemeral=True)


class RemoveRoleFromGroupModal(discord.ui.Modal):
    def __init__(self):
        super().__init__(title="Убрать роль из группы")
        self.add_item(discord.ui.InputText(
            label="ID группы",
            placeholder="Например: 12",
            required=True,
    ...
    async def callback(self, interaction):
        ...
        if role.id not in role_ids:
            return await interaction.response.send_message(f"ℹ️ Роли {role.mention} нет в группе {group.title}.", ephemeral=True)

        role_ids.remove(role.id)
        set_role_ids_for_group(group, role_ids)
        await refresh_online_after_setting(interaction)
        await interaction.response.send_message(f"✅ Роль {role.mention} удалена из группы {group.title}.", ephemeral=True)
...
class RenameGroupModal(discord.ui.Modal):
    def __init__(self):
        ...
        self.add_item(discord.ui.InputText(
            ...
            placeholder="Например: 🔥 Проверенные бойцы",
            required=True,
            max_length=100,
        ))
    ...
    async def callback(self, interaction):
        ...
        group_id = parse_positive_int(self.children[0].value)
        new_title = self.children[1].value.strip()

        if not group_id:
            return await interaction.response.send_message("❌ Укажи корректный ID группы.", ephemeral=True)
        ...
        group.save()
        await refresh_online_after_setting(interaction)
        await interaction.response.send_message(f"✅ Группа переименована: {old_title} → {group.title}", ephemeral=True)
...
class MoveGroupModal(discord.ui.Modal):
    ...
    async def callback(self, interaction):
        ...
        group_id = parse_positive_int(self.children[0].value)
        position = parse_positive_int(self.children[1].value)

        if not group_id or not position or position < 1:
            return await interaction.response.send_message("❌ Укажи корректный ID группы и позицию 1 или больше.", ephemeral=True)
...
class DeleteGroupModal(discord.ui.Modal):
    ...
    async def callback(self, interaction):
        ...
        group_id = parse_positive_int(self.children[0].value)
        confirm = self.children[1].value.strip().upper()

        if confirm != "УДАЛИТЬ":
            return await interaction.response.send_message("❌ Удаление отменено. Для подтверждения нужно написать `УДАЛИТЬ`.", ephemeral=True)
...
class SteamMissingRoleSetModal(discord.ui.Modal):
    def __init__(self):
        super().__init__(title="Задать роль проверки Steam")
        self.add_item(discord.ui.InputText(
            label="ID роли или упоминание роли",
            placeholder="Например: @Участник клана или 123456789012345678",
            required=True,
            max_length=100,
        ))
    ...
    async def callback(self, interaction):
        ...
        role_id = parse_discord_id(self.children[0].value)
        if not role_id:
            return await interaction.response.send_message("❌ Укажи корректный ID роли или упоминание роли.", ephemeral=True)
...
class OnlineSettingsButtonView(discord.ui.View):
    def __init__(self):
        super().__init__(timeout=300)

    @discord.ui.button(label="➕ Добавить группу", style=discord.ButtonStyle.green)
    async def add_group_button(self, button, interaction):
        if not can_manage_online(interaction.user):
            return await deny_interaction(interaction)
        await interaction.response.send_modal(AddGroupModal())

    @discord.ui.button(label="➕ Добавить роль", style=discord.ButtonStyle.blurple)
    async def add_role_button(self, button, interaction):
        if not can_manage_online(interaction.user):
            return await deny_interaction(interaction)
        await interaction.response.send_modal(AddRoleToGroupModal())

    @discord.ui.button(label="➖ Убрать роль", style=discord.ButtonStyle.gray)
    async def remove_role_button(self, button, interaction):
        if not can_manage_online(interaction.user):
            return await deny_interaction(interaction)
        await interaction.response.send_modal(RemoveRoleFromGroupModal())

    @discord.ui.button(label="✏️ Переименовать", style=discord.ButtonStyle.gray)
    async def rename_group_button(self, button, interaction):
        if not can_manage_online(interaction.user):
            return await deny_interaction(interaction)
        await interaction.response.send_modal(RenameGroupModal())

    @discord.ui.button(label="↕️ Порядок", style=discord.ButtonStyle.gray)
    ...
    @discord.ui.button(label="🗑️ Удалить группу", style=discord.ButtonStyle.red)
    ...
    async def steam_role_button(self, button, interaction):
        if not can_manage_online(interaction.user):
            return await deny_interaction(interaction)
        await interaction.response.send_modal(SteamMissingRoleSetModal())

    @discord.ui.button(label="🧹 Убрать роль Steam", style=discord.ButtonStyle.red)
    async def clear_steam_role_button(self, button, interaction):
        if not can_manage_online(interaction.user):
            return await deny_interaction(interaction)
        clear_steam_required_role_id(interaction.guild.id)
        await interaction.response.send_message("✅ Отдельная роль проверки Steam убрана.", ephemeral=True)

    @discord.ui.button(label="📋 Показать список", style=discord.ButtonStyle.blurple)
    async def list_groups_button(self, button, interaction):
        if not can_manage_online(interaction.user):
            return await deny_interaction(interaction)
...
async def online_group_add(
    ...
    await ctx.respond(
        ...
        f"Место в иерархии: `{group.position}`\n"
...
async def online_group_section(ctx, group_id: int, section: str):
    ...
    await ctx.respond(
        "ℹ️ Разделы отключены. Теперь у бота единая иерархия групп. "
        "Используй `/online_group_move`, чтобы менять порядок группы.",
        ephemeral=True,
    )
