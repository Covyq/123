...
def get_role_icon_text(role):
    """
    Возвращает отображаемый значок роли для текста embed.
    Если у роли стоит unicode-эмодзи, он будет показан прямо перед ролью.
    Если у роли стоит картинка-иконка, Discord не умеет вставлять её как изображение
    внутри строки embed, поэтому показывается кликабельный значок 🖼️ со ссылкой.
    """
...
def build_group_role_member_chunks(guild, online_members, group, max_len=1000):
    ...
    а каждую добавленную роль, значок роли и участников этой роли.
    ...
    for role_id in get_role_ids_from_group(group):
        ...
        role_icon = get_role_icon_text(role)
        blocks.append(
            f"**{role_icon}{role.mention} — {len(role_members)}**\n"
